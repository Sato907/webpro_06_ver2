...
// トップページ（制作した各サイトのまとめ一覧）
app.get("/", (req, res) => {
  const sites = [
    { title: "哲学者一覧",   href: "/manga",    color: "#6c5ce7", desc: "ニーチェなど哲学者を紹介する一覧サイト（登録・編集・削除）" },
    { title: "教育学者一覧", href: "/kyouiku",  color: "#00b894", desc: "教育の偉人をまとめた一覧サイト（登録・編集・削除）" },
    { title: "美少女一覧",   href: "/bisyoujo", color: "#e84393", desc: "美少女キャラクターの一覧サイト（登録・編集・削除）" },
    { title: "おすすめ診断", href: "/sindan",   color: "#0984e3", desc: "質問に答えてピッタリの一人を見つける診断サイト" },
  ];
...
app.get("/manga/:number", (req, res) => {
  ...
  res.render('manga_detail2', {name: number, data: detail} );
});
...
app.get("/kyouiku/:number", (req, res) => {
  ...
  res.render('kyouiku_detail2', {name: number, data: detail} );
});
...
app.get("/bisyoujo/:number", (req, res) => {
  ...
  res.render('bisyoujo_detail2', {name: number, data: detail} );
});
